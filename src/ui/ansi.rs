...
fn append_style(s: &mut String, prev: &crate::terminal_emulator::Style, next: &crate::terminal_emulator::Style) {
    ...
    if prev.attrs.has(Attributes::REVERSE) != next.attrs.has(Attributes::REVERSE) {
        s.push_str(if next.attrs.has(Attributes::REVERSE) { REVERSE } else { REVERSE_OFF });
    }
    if prev.fg != next.fg {
