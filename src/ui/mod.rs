...
pub use render::{Frame, compute_render_state, render};
pub use terminal_view::{
    draw_emulator_content, draw_shell_content, draw_terminal_content, terminal_content_width,
};
...
fn tab_content_char(title: &str, content_rows: usize, row: usize, scroll_offset: usize) -> char {
    ...
    if len == 0 {
        ...
        chars.get(row).copied().unwrap_or(' ')
    } else {
        chars[(scroll_offset + row) % len]
    }
}
...
pub fn scrollbar_thumb(
    ...
    let thumb_len =
        (((visible as f32 / total as f32) * track_len as f32).max(1.0) as usize).min(track_len);
